...
class historyList : public history<T> {
    protected:
        std::vector<T> historyList;
        typename std::vector<T>::iterateur cursor;
    public:
        historyList() {
            cursor = history_list.begin() - 1;
        }
        virtual void put(const T &x) override {
                history_list.push_back(x);
                cursor = history_list.end() + 1;
        }
        virtual bool empty() const  override {
            return history_list.empty();
        }
        virtual bool before() const override {
            return cursor == history_list.begin();
        }
        virtual bool after() const override {
            return cursor == history_list.end();
        }
        virtual bool is_first() const override {
            return cursor == history_list.begin();
        }
        virtual bool is_last() const override {
            return cursor == history_list.end() - 1;
        }
        ...
        virtual void remove_all_right() override {
                history_list.erase(cursor + 1, history_list.end());
        }
}
